...
class TestUnitTests(unittest.TestCase):
    ...
    def assertQueueAttributes(self, expected, actual, msg=None):
        ...
        if not expected.items() <= actual.items():
            diff = "\n" + "\n".join(
                difflib.ndiff(
                    pprint.pformat(expected).splitlines(), pprint.pformat(actual_short).splitlines()
                )
            )
            standardMsg = "%s !<= %s" % (safe_repr(expected), safe_repr(actual_short))
    ...
    def test_qname_good_validation(self):
        ...
        for qname in GOOD_QUEUE_NAMES:
            self.assertTrue(queue.get_queue_attr().qname(qname).ready())
    ...
    def test_qname_bad_validation(self):
        ...
        client.flushall()
        queue = RedisSMQ(client=client, exceptions=False)
        BAD_QUEUE_NAMES = ["", None, "something:else", ":", "something:", ":else"]

        for qname in BAD_QUEUE_NAMES:
            self.assertFalse(queue.get_queue_attr().qname(qname).ready())
    ...
    def test_queue_attributes(self):
        ...
        self.assertSetEqual(set([queue_name]), queues)
        queue_details = client.hgetall(queue_key)
        self.assertQueueAttributes({"vt": "15", "delay": "10", "maxsize": "65565"}, queue_details)
        attributes = queue.get_queue_attr(qname=queue_name).execute()
        ...
        attributes_1 = queue.setQueueAttributes(vt=30, maxsize=1024, delay=1).execute()
        attributes = queue.get_queue_attr(qname=queue_name).execute()
    ...
    def test_invalid_queue_attributes(self):
        ...
        attributes_1 = queue.setQueueAttributes(vt=-3, maxsize=-2, delay=-1).execute()
        attributes = queue.get_queue_attr(qname=queue_name).execute()
        self.assertDictEqual(attributes, attributes_1)
        self.assertQueueAttributes(
            {
