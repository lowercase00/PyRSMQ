...
class BaseRSMQCommand:
    ...
    def exceptions(self, enabled=True):
        """Control exceptions"""
        self.__exceptions = enabled is not False
        return self
    ...
    def _validate_maxsize(self, maxsize):
        ...
        return maxsize == -1 or validate_int(
            maxsize, const.MAXSIZE_MIN, const.MAXSIZE_MAX, logger=self.log, name="maxsize"
        )
    ...
    def execute(self):
        ...
        try:
            ...
            if self.get_quiet is not True:
                self.log.warning(
                    "%s: Exception while processing %s: %s",
