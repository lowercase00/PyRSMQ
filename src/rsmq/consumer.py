...
class RedisSMQConsumer:
    ...
    class VisibilityTimeoutExtender(Thread):
        ...
        def __init__(self, consumer, message_id):
            ...
            super(RedisSMQConsumer.VisibilityTimeoutExtender, self).__init__(name=thread_name)
        ...
        def extend(self):
            ...
            try:
                ...
                self.rsqm.changeMessageVisibility(vt=self.vt, id=self.message_id).execute()
                self.set_next_extension()
            except RedisSMQException as ex:
                LOG.warning("Failed to extend message visibility for %s: %s", self.message_id, ex)
    ...
    def _get_vt(self):
        ...
        if self.vt is None:
            queue_info = (
                self.rsqm.getQueueAttributes().qname(self.qname).exceptions(False).execute()
            )
    ...
    def on_failure(self, msg):
        ...
        if msg and "id" in msg:
            ...
            self.rsqm.changeMessageVisibility(
                vt=self.retry_delay, qname=self.qname, id=msg["id"]
            ).execute()
    ...
    def run(self):
        ...
        while not self._request_stop:
            try:
                ...
                if msg and "id" in msg:
                    extender = RedisSMQConsumer.VisibilityTimeoutExtender(self, msg["id"])
                    extender.start()
                    ...
                    raise RedisSMQException("Invalid message in queue '%s': %s" % (self.qname, msg))
